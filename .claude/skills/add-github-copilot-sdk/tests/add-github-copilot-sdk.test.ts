...
describe('add-github-copilot-sdk applied changes', () => {
  ...
  describe('.claude/skills/x-integration/agent.ts', () => {
    const content = fs.readFileSync(
      path.join(projectRoot, '.claude', 'skills', 'x-integration', 'agent.ts'),
      'utf-8',
    );

    it('imports defineTool from @github/copilot-sdk', () => {
      expect(content).toContain("from '@github/copilot-sdk'");
      expect(content).toContain('defineTool');
    });

    it('uses defineTool(name, config) signature', () => {
      expect(content).toContain("defineTool('x_post'");
      expect(content).toContain("defineTool('x_like'");
      expect(content).toContain("defineTool('x_reply'");
      expect(content).toContain("defineTool('x_retweet'");
      expect(content).toContain("defineTool('x_quote'");
    });

    it('returns string results from handlers', () => {
      // Handlers return plain strings, not { content, isError } objects
      expect(content).toContain('return result.message');
      expect(content).toContain("return 'Error:");
    });

    it('does not use old Claude SDK tool() API', () => {
      expect(content).not.toContain("from '@anthropic-ai");
      // Make sure we're not using the old tool() function (but allow defineTool)
      expect(content).not.toMatch(/\btool\s*\(\s*['"][^'"]+['"]/);
    });
  });
