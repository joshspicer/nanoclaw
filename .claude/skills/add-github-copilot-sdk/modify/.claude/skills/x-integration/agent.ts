...
/**
 * Create X integration tools for the Copilot SDK.
 * Uses defineTool(name, config) signature per SDK docs.
 * Handler returns string result directly.
 */
export function createXTools(ctx: SkillToolsContext): Tool[] {
...
Make sure the content is appropriate and within X's character limit (280 chars for text).`,
      ...
      handler: async (args: { content: string }) => {
        if (!isMain) {
          return 'Error: Only the main group can post tweets.';
        }

        if (args.content.length > 280) {
          return `Error: Tweet exceeds 280 character limit (current: ${args.content.length})`;
        }
...
Provide the tweet URL or tweet ID to like.`,
      ...
      handler: async (args: { tweet_url: string }) => {
        if (!isMain) {
          return 'Error: Only the main group can interact with X.';
        }
